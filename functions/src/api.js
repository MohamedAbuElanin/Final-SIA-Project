...
// 4. AI Analysis
// FIXED: Unifies analysis trigger and ensures Single Source of Truth
// Endpoint: POST /api/analyze-with-gemini
app.post("/analyze-with-gemini", authenticateUser, async (req, res) => {
  try {
    const uid = req.user.uid;

    // 1. Fetch Completion Status & Scores from Single Source of Truth
    const resultsRef = admin.firestore().collection("tests_results").doc(uid);
    ...
    // Support { holland: { completed: true }, bigFive: { ... } }
    // AND legacy { hollandCode: ..., bigFive: ... } if existing,
    // BUT strict requirement says use tests_results/{uid} ONLY.
    ...
    if (!bigFiveCompleted || !hollandCompleted) {
      return res.status(400).json({
        ...
        message: "Both Big Five and Holland tests must be completed before analysis.",
      });
    }
    ...
    // 3. Generate Analysis
    // We pass the raw scores or results stored in tests_results
    const bigFiveResults = data.bigFive?.scores || data.bigFive?.result || data.bigFive;
    const hollandResults = data.holland?.scores || data.holland?.result || data.holland;

    let analysis = {};

    try {
      // Generate combined analysis with Matching Engine
      const [bfAnalysis, hAnalysis, careerAnalysis] = await Promise.all([
        generateAnalysis("Big-Five", bigFiveResults, userProfile),
        generateAnalysis("Holland", hollandResults, userProfile),
        generateAnalysis("Career-Match", {bigFive: bigFiveResults, holland: hollandResults}, userProfile),
      ]);
      ...
      analysis = {
        ...
        typeExplanation: hAnalysis.typeExplanation || "Your Holland Type Analysis",
        topCareers: careerAnalysis.topCareers || [], // From Matching Engine + AI
        learningRecommendations: bfAnalysis.learningRecommendations || [],
        generatedAt: admin.firestore.FieldValue.serverTimestamp(),
      };
    } catch (aiError) {
      logger.error("Gemini Generation failed:", aiError.message);
      return res.status(503).json({error: "AI Service Unavailable", message: aiError.message});
    }

    // 4. Save to users/{uid} (MANDATORY)
    ...
    logger.error("Error in analyze-with-gemini:", error.message);
    res.status(500).json({
      error: "Internal server error",
